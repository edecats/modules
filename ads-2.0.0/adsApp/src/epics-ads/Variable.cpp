...
uint32_t Variable::size() {
    return this->elem_size * this->addr->get_nelem();
}

void Variable::set_connection(std::shared_ptr<Connection> connection) {
    this->conn = connection;
}

std::shared_ptr<Connection> Variable::get_connection() {
    return this->conn;
}

void Variable::set_buffer_reader(BufferDataPosition br) {
    this->buffer_reader = br;
}

BufferDataPosition Variable::get_buffer_reader() {
    return this->buffer_reader;
}

bool Variable::notifies_on_update() {
    return this->notify_on_update;
}

void Variable::set_notify_on_update(const bool notify) {
    this->notify_on_update = notify;
}

Variable::Variable(std::string address, bool notify_on_update) : notify_on_update(notify_on_update), addr(new ADSAddress(address)) {
    this->elem_size = ads_datatype_sizes[this->addr->get_data_type()];
}

Variable::Variable(std::shared_ptr<ADSAddress> address, bool notify_on_update) : notify_on_update(notify_on_update), addr(address) {
    this->elem_size = ads_datatype_sizes[this->addr->get_data_type()];
}

int Variable::read_from_buffer(const uint32_t size, uint8_t *buffer) {
    ...
    if (this->buffer_reader.buffer->buffer_state == SumReadBuffer::SumReadBufferState::Invalid) {
    ...
    int rc = buffer_reader.buffer->read_data(
            buffer_reader.off_result,
            buffer_reader.off_data,
            bytes_to_read,
            &read_result,
            buffer);
    ...
    if (read_result != 0) {
        LOG_WARN("variable '%s' could not be read (%u): %s", this->addr->info().c_str(), read_result, errorMap[read_result].c_str());
        return EPICSADS_NO_DATA;
    }
...
std::pair<uint8_t *, RWLock *> Variable::get_read_data() {
    if (this->buffer_reader == EMPTY_BUFFER_DATA_POSITION) {
...
int Variable::write(const uint8_t *data, const uint32_t size) {
    if (data == nullptr) {
    ...
    struct AmsAddr remote_ams_addr = {
            this->conn->get_remote_ams_netid(),
            this->addr->get_ads_port()
        };

    long rc = AdsSyncWriteReqEx(
...
int Variable::read(uint8_t *data, const uint32_t size, uint32_t *bytes_read) {
    if (data == nullptr) {
        return EPICSADS_INV_PARAM;
    }
