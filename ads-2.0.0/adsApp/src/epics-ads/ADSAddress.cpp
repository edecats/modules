...
ADSDataType ADSAddress::get_data_type() {
    return this->data_type;
}

std::string ADSAddress::get_var_name() {
    return this->variable_name;
}

Operation ADSAddress::get_operation() {
    return this->operation;
}

uint16_t ADSAddress::get_ads_port() {
    return this->ads_port;    
}

uint32_t ADSAddress::get_index_group() {
    return this->index_group;
}

uint32_t ADSAddress::get_index_offset() {
    return this->index_offset;
}

uint32_t ADSAddress::get_nelem() {
    return this->nelem;
}

uint32_t ADSAddress::get_notification_delay() {
    return this->ads_notification_delay;
}

bool ADSAddress::is_resolved() {
    return this->name_is_resolved;
}
...
const std::string ADSAddress::info() {
    ...
    o << "[" << this->get_nelem() << " elem/" << (this->get_nelem() * ads_datatype_sizes[this->get_data_type()]) << " bytes] ";
    o << "P=" << this->get_ads_port() << " ";

    if (this->get_var_name() != "") {
        o << "V=" << std::hex << this->get_var_name() << " (handle G=0x" << this->get_index_group() << " O=0x" << this->get_index_offset() << ")";
    } else {
        o << "G=0x" << std::hex << this->get_index_group() << " O=0x" << this->get_index_offset();
    }
    o << std::dec;
...
ADSAddress::ADSAddress(const std::string address) {
    ...
    if (this->data_type == ADSDataType::STRING && this->nelem == 0) {
        throw std::invalid_argument("Missing NELEM specifier for STRING datatype");
    }
...
int ADSAddress::resolve(const uint32_t ads_index_group, const uint32_t ads_index_offset) {
...
void ADSAddress::parse_register_specifier(std::vector<std::string>& address_tokens) {
    ...
    if (address_tokens.size() >= 6) {
        this->ads_notification_delay = parse_notification_delay(address_tokens[5]);
    }
}

void ADSAddress::parse_variable_specifier(std::vector<std::string>& address_tokens) {
    if (address_tokens.size() < 4) {
    ...
    /* Notification delay is an optional parameter */
    if (address_tokens.size() >= 5) {
        this->ads_notification_delay = parse_notification_delay(address_tokens[4]);
    }
}
...
static uint32_t parse_nelem(const std::string s) {
    ...
    try {
        value = s.substr(s.find("["), std::string::npos);
    ...
    /* Start with index 1 to skip the first '[' character */
    value = value.substr(1, value.find("]"));
    ...
    return (uint32_t) nelem;
}

/* Parse ADS port specifier. Expected format: "P=PORT", where PORT is an unsigned
 * integer or a string key from ads_port_map (e.g. "PLC_TC3").
 *
