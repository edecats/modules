...
struct ReadRequestChunk {
    ...
    void add_variable(std::shared_ptr<Variable> variable) {
        ...
        if (rc != 0) {
            throw std::length_error("could not add variable to sum-read buffer");
        }

        this->sum_read_request_buffer.push_back({0, 0, 0});

        this->variables.push_back(std::shared_ptr<Variable>(variable));
    }
};

int SumReadRequest::get_num_chunks() {
    return this->get_chunks()->size();
}

bool SumReadRequest::is_allocated() {
    return this->allocated;
}

bool SumReadRequest::is_initialized() {
    return this->initialized;
}

SumReadRequest::SumReadRequest(const uint16_t max_variables_per_buffer, std::shared_ptr<Connection> connection) : conn(connection) {
    if (max_variables_per_buffer == 0) {
        throw std::invalid_argument("max_variables_per_buffer must be larger than zero");
    }
...
std::shared_ptr<std::vector<std::shared_ptr<ReadRequestChunk>>> SumReadRequest::get_chunks() {
    auto v = std::make_shared<std::vector<std::shared_ptr<ReadRequestChunk>>>();

    for (auto chunk_set_itr = this->chunks_by_ads_port.begin(); chunk_set_itr != this->chunks_by_ads_port.end(); chunk_set_itr++) {
        std::shared_ptr<std::vector<std::shared_ptr<ReadRequestChunk>>> chunk_set = chunk_set_itr->second;
        for (auto chunk_itr = chunk_set->begin(); chunk_itr != chunk_set->end(); chunk_itr++) {
...
int SumReadRequest::allocate(std::vector<std::shared_ptr<Variable>>& variables) {
    if (this->is_allocated() == true) {
        return EPICSADS_INV_CALL;
    }

    for (auto var_itr = variables.begin(); var_itr != variables.end(); var_itr++) {
        /* Get variable and ADS port which it uses */
        std::shared_ptr<Variable> var = *var_itr;
        uint32_t ads_port = var->addr->get_ads_port();

        /* Create chunk set for the ADS port if necessary */
        if (this->chunks_by_ads_port.find(ads_port) == this->chunks_by_ads_port.end()) {
            this->chunks_by_ads_port[ads_port] = std::make_shared<std::vector<std::shared_ptr<struct ReadRequestChunk>>>();
        }

        /* Get chunk set for ADS port */
        std::shared_ptr<std::vector<std::shared_ptr<ReadRequestChunk>>> chunk_set = this->chunks_by_ads_port[ads_port];

        /* Create read request chunk if it doesn't exist or if its read buffer
         * is at full capacity  */
        if (chunk_set->size() == 0 || chunk_set->back()->sum_read_data_buffer->at_capacity() == true) {
            chunk_set->push_back(std::make_shared<struct ReadRequestChunk>(ads_port, this->max_vars_per_buffer));
        }
    ...
    for (auto chunk_set_itr = this->chunks_by_ads_port.begin(); chunk_set_itr != this->chunks_by_ads_port.end(); chunk_set_itr++) {
        ...
        for (auto chunk_itr = chunk_set->begin(); chunk_itr != chunk_set->end(); chunk_itr++) {
            ...
            if (rc != 0) {
                LOG_ERR("failed to initialize sum-read data buffer (%i): %s", rc, ads_errors[rc].c_str());
                goto ERROR;
            }
        }
    }
...
int SumReadRequest::deallocate() {
    ...
    for (auto chunk_set_itr = this->chunks_by_ads_port.begin(); chunk_set_itr != this->chunks_by_ads_port.end(); chunk_set_itr++) {
        ...
        for (auto chunk_itr = chunk_set->begin(); chunk_itr != chunk_set->end(); chunk_itr++) {
            std::shared_ptr<ReadRequestChunk> chunk = *chunk_itr;
            for (auto var_itr = chunk->variables.begin(); var_itr != chunk->variables.end(); var_itr++) {
...
int SumReadRequest::initialize() {
    ...
    for (auto chunk_itr = chunk_set->begin(); chunk_itr != chunk_set->end(); chunk_itr++) {
        ...
        for (size_t i_var = 0; i_var < chunk->variables.size(); i_var++) {
            std::shared_ptr<Variable> var = chunk->variables[i_var];
            if (var->addr->is_resolved() == false) {
                LOG_ERR("variable name is not resolved: '%s'", var->addr->get_var_name().c_str());
                return EPICSADS_NOT_RESOLVED;
            }

            chunk->sum_read_request_buffer[i_var] = {
                    var->addr->get_index_group(),
                    var->addr->get_index_offset(),
                    var->size()
                };
        }
    }
...
int SumReadRequest::deinitialize() {
    ...
    for (auto chunk_itr = chunk_set->begin(); chunk_itr != chunk_set->end(); chunk_itr++) {
        std::shared_ptr<ReadRequestChunk> chunk = *chunk_itr;
        for (size_t i_var = 0; i_var < chunk->sum_read_request_buffer.size(); i_var++) {
...
void SumReadRequest::set_buffers_state(SumReadBuffer::SumReadBufferState state) {
    auto chunk_set = this->get_chunks();
    for (auto chunk_itr = chunk_set->begin(); chunk_itr != chunk_set->end(); chunk_itr++) {
...
int SumReadRequest::read() {
    ...
    auto chunk_set = this->get_chunks();
    for (auto chunk_itr = chunk_set->begin(); chunk_itr != chunk_set->end(); chunk_itr++) {
        std::shared_ptr<SumReadBuffer> sum_read_data_buffer = (*chunk_itr)->sum_read_data_buffer;
        ...
        uint32_t write_buffer_size = (*chunk_itr)->sum_read_request_buffer.size() * sizeof(AdsSymbolInfoByName);
        uint8_t *write_buffer = (uint8_t *)(*chunk_itr)->sum_read_request_buffer.data();
        ...
        AmsAddr remote_ams_addr = {
                this->conn->get_remote_ams_netid(),
                (*chunk_itr)->ads_port
            };
        ...
        long rc = AdsSyncReadWriteReqEx2(
                ...
                read_buffer, // read data buffer (written by PLC), where the read operation results and data is stored
                write_buffer_size, // write buffer size in bytes
                write_buffer, // write buffer (data sent to PLC), containing read requests for PLC variables
        ...
        sum_read_data_buffer->buffer_state = SumReadBuffer::SumReadBufferState::Valid;
    }

    return 0;
}

std::vector<std::shared_ptr<Variable>> SumReadRequest::get_updated_variables() {
    ...
    auto chunk_set = this->get_chunks();
    for (auto chunk_itr = chunk_set->begin(); chunk_itr != chunk_set->end(); chunk_itr++) {
        auto updated_vars = (*chunk_itr)->sum_read_data_buffer->get_updated_variables();
        for (auto var = updated_vars.begin(); var != updated_vars.end(); var++) {
            vars.push_back(*var);
        }
    }
...
void SumReadRequest::print_info(FILE *fd, int details) {
    if (details >= 1) {
        ...
        fprintf(fd, " Summary:\n");
        fprintf(fd, "   - Max number of variables per sum-request and sum-read buffer: %u\n", this->max_vars_per_buffer);
        fprintf(fd, "   - Number of sum-request and sum-read buffers (chunks) allocated: %i\n", this->get_num_chunks());
        ...
        for (size_t i_chunk = 0; i_chunk < chunk_set->size(); i_chunk++) {
            ...
            if (details >= 3) {
                ...
                for (size_t i_var = 0; i_var < chunk->variables.size(); i_var++) {
                    ...
                    fprintf(fd, "    - Variable %lu/%lu:\n", (i_var + 1), chunk->variables.size());
                    fprintf(fd, "       - Name: '%s'\n", var->addr->info().c_str());
                    fprintf(fd, "       - Port: %i; IGrp: %#09x; Ioff: %#09x; Length: %u\n", var->addr->get_ads_port(), req_info.indexGroup, req_info.indexOffset, req_info.cbLength);
                }
            }
        }
    }
