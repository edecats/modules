...
public:
     ...
     * symbolic name. */
    int resolve_variable(std::shared_ptr<Variable> ads_variable);

    /* Resolve (acquire handles) for ADS variables specified with a symbolic
     * name. */
    int resolve_variables(const std::vector<std::shared_ptr<Variable>>& ads_variables);

    /* Unresolve (release handle) for a single ADS variable specified with a
     * symbolic name. */
    int unresolve_variable(std::shared_ptr<Variable> ads_variable);

    /* Unresolve (release handles) for ADS variables specified with a symbolic
     * name. */
    int unresolve_variables(const std::vector<std::shared_ptr<Variable>>& ads_variables);
    ...
    int read_device_info(char *device_info, size_t size_device_info, AdsVersion *ads_version);
