...
unsigned int SumReadBuffer::get_max_variables() {
    return this->max_nvariables;
}
...
size_t SumReadBuffer::get_size() {
    return this->buffer_size;
}

bool SumReadBuffer::is_initialized() {
    return this->buffer_initialized;
}

bool SumReadBuffer::at_capacity() {
    return (this->data_size() >= this->get_max_data_size_soft_limit()) || (this->next_result_offset >= this->max_nvariables);
}

uint8_t *SumReadBuffer::get_buffer() {
    return this->buffer;
}
...
SumReadBuffer::SumReadBuffer(const uint16_t max_nvariables) : rwlock(RWLock::prefer_writer) {
    if (max_nvariables == 0) {
...
SumReadBuffer::SumReadBuffer(const uint16_t max_nvariables, const size_t data_size_soft_limit) : SumReadBuffer(max_nvariables) {
    if (data_size_soft_limit == 0) {
        throw std::invalid_argument("data_size_soft_limit must be larger that zero");
    }
...
int SumReadBuffer::add_variable(std::shared_ptr<Variable> variable) {
     ...
     * found in the buffer */
    BufferDataPosition bdp = { this, this->next_result_offset, this->next_data_offset };
...
int SumReadBuffer::read_data(uint16_t offset_result, size_t offset_data, const size_t data_size, uint32_t *result, uint8_t *data) {
    if (this->buffer == nullptr) {
        return EPICSADS_NOT_INITIALIZED;
    ...
    *result = *buffer_rc;
    ...
    uint8_t *buffer_data = this->buffer + this->start_of_data_offset() + offset_data;
...
int SumReadBuffer::initialize_buffer() {
    ...
    this->prev_data_buffer = (uint8_t *)calloc(this->buffer_size, sizeof(uint8_t));
...
std::vector<std::shared_ptr<Variable>> SumReadBuffer::get_updated_variables() {
    std::vector<std::shared_ptr<Variable>> updated_vars;

    for (auto it = this->notify_on_update_vars.begin(); it != this->notify_on_update_vars.end(); it++) {
        ...
        if (*read_result != 0) {
            LOG_WARN("variable '%s' could not be read (%u): %s", (*it)->addr->info().c_str(), *read_result, errorMap[*read_result].c_str());
            continue;
        }

        uint8_t *new_values_buffer_start = this->buffer + this->start_of_data_offset() + bdp.off_data;
        uint8_t *prev_values_buffer_start = this->prev_data_buffer + this->start_of_data_offset() + bdp.off_data;

        if (memcmp(new_values_buffer_start, prev_values_buffer_start, (*it)->size()) != 0) {
...
size_t SumReadBuffer::start_of_data_offset() {
