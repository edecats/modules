...
bool Connection::is_connected() {
    return (this->ads_port != 0 ? true : false);
}
...
int Connection::resolve_variable(std::shared_ptr<Variable> ads_variable) {
...
int Connection::resolve_variables(const std::vector<std::shared_ptr<Variable>>& ads_variables) {
    std::lock_guard<epicsMutex> lock(this->mtx);
    ...
    for (size_t i = 0; i < ads_variables.size(); i++) {
        std::shared_ptr<Variable> ads_var = ads_variables[i];
        ...
        if (rc != 0) {
            LOG_WARN("could not resolve ADS variable '%s'", ads_var->addr->get_var_name().c_str());
...
int Connection::unresolve_variable(std::shared_ptr<Variable> ads_variable) {
    std::vector<std::shared_ptr<Variable>> vec;
    vec.push_back(ads_variable);
...
int Connection::unresolve_variables(const std::vector<std::shared_ptr<Variable>>& ads_variables) {
    std::lock_guard<epicsMutex> lock(this->mtx);
...
int Connection::read_device_info(char *device_info, size_t size_device_info, AdsVersion *ads_version) {
    ...
    long rc = AdsSyncReadDeviceInfoReqEx(
            this->ads_port, // ADS port
            &ams_addr, // AMS address
