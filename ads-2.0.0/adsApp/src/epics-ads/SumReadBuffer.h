...
protected:
    ...
    std::vector<std::shared_ptr<Variable>> notify_on_update_vars;

    uint16_t max_nvariables = 0; /* Maximum allowed number of variables */
    size_t max_data_size_soft_limit = 1000000; /* Maximum soft limit for data segment size */
    uint16_t next_result_offset = 0; /* Buffer index for the next added result */
    size_t next_data_offset = 0; /* Buffer byte offset from the beginning of the data block */
     ...
     * at the beginning of the buffer and are all of the same size. Data elements
     * are stored after the results block and are of varying size. */
    ...
    size_t start_of_data_offset(); /* Byte offset where data elements are stored in buffer */


public:
    enum class SumReadBufferState { Valid, Invalid } buffer_state = SumReadBufferState::Invalid;
    SumReadBuffer(const uint16_t max_nvariables);
    SumReadBuffer(const uint16_t max_nvariables, const size_t data_size_soft_limit);
    ~SumReadBuffer();
    ...
    size_t get_max_data_size_soft_limit(); /* Soft limit for maximum data segment size in bytes */
     ...
     * before attempting to manipulate the buffer and release the lock when done. */
    uint8_t *get_buffer();
    ...
    int add_variable(std::shared_ptr<Variable> variable);
     ...
     * underlying buffer. */
    int read_data(uint16_t offset_result, size_t offset_data, const size_t data_size, uint32_t *result, uint8_t *data);
