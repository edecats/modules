...
protected:
    ...
    std::shared_ptr<std::vector<std::shared_ptr<ReadRequestChunk>>> get_chunks();
...
public:
    ...
    SumReadRequest(const uint16_t max_variables_per_buffer, std::shared_ptr<Connection> connection);
    ~SumReadRequest();
    ...
    int allocate(std::vector<std::shared_ptr<Variable>>& variables);
